...
class ContentRepository {
     ...
     */
    async schedulePresentedPhrasesForReview(userId, content) {
        if (mongoose_1.default.connection.readyState !== 1 || !mongoose_1.default.Types.ObjectId.isValid(userId))
            return;
        const now = new Date();
        ...
        for (const item of candidates) {
            const existing = await vocabularyItem_model_1.VocabularyItemModel.findOne({ userId, phrase: item.phrase }).select("_id").lean();
            if (existing)
                continue;
            const saved = await vocabularyItem_model_1.VocabularyItemModel.findOneAndUpdate({ userId, phrase: item.phrase }, {
                $setOnInsert: {
                    userId,
                    phrase: item.phrase,
                    translation: item.translation,
    ...
    async generateReviewQueue(userId) {
        if (mongoose_1.default.connection.readyState !== 1 || !mongoose_1.default.Types.ObjectId.isValid(userId))
            return [];
        ...
        const reviewItems = await vocabularyItem_model_1.VocabularyItemModel.find({ userId, phrase: { $in: phrases } })
            .sort({ misses: -1, confidence: 1, updatedAt: -1 })
            .limit(7)
            .select("_id");
        if (reviewItems.length > 0) {
            await reviewSchedule_model_1.ReviewScheduleModel.updateMany({ userId, vocabularyItemId: { $in: reviewItems.map((item) => item._id) } }, { $set: { nextReviewAt: new Date() } });
        }
