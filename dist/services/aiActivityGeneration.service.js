...
const hasUsableContent = (activity) => {
    if (!activity.generatedContent || typeof activity.generatedContent !== "object")
        return false;
    const content = activity.generatedContent;
    if (activity.module === "listening")
        return Array.isArray(content.dialogue) && content.dialogue.length > 0;
    if (activity.module === "shadowing")
...
class AiActivityGenerationService {
    ...
    async getOrGenerate(plan, activityId) {
        ...
        if (activity.status === "ready" && hasUsableContent(activity))
