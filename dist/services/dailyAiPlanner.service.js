...
const localFallback = (plan, level) => ({
    ...
    activities: plan.blocks.slice(0, 5).map((block, index) => ({
        id: `${plan.id}:local-${block.type}-${index + 1}`, module: block.type === "speaking-coach" ? "pronunciation" : block.type === "review" ? "vocabulary" : block.type,
