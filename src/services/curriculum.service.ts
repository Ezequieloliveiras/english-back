import { EnglishLevel, ProgressSnapshot, UserProfile } from "../types";

export type LearningSkill = "listening" | "speaking" | "vocabulary" | "pronunciation" | "interaction";
...
export interface ProficiencyAssessment {
  /** A working estimate, never a formal CEFR certification. */
  estimatedLevel: EnglishLevel;
  confidence: "insufficient" | "early" | "developing" | "supported";
  calibrated: boolean;
  evidenceCount: number;
  assessedAt: string;
  skillEvidence: Record<LearningSkill, { score: number | null; observations: number; status: "not_enough_data" | "developing" | "secure" }>;
  nextCompetencies: CurriculumCompetency[];
  explanation: string;
}

const levelOrder: EnglishLevel[] = ["A1", "A2", "B1", "B2", "C1"];

export const CEFR_CURRICULUM: CurriculumCompetency[] = [
...
const scoreStatus = (score: number | null, observations: number): "not_enough_data" | "developing" | "secure" =>
  score === null || observations < 3 ? "not_enough_data" : score >= 70 && observations >= 6 ? "secure" : "developing";
 ...
 */
export const assessProficiency = (input: {
  profile: UserProfile;
  progress: ProgressSnapshot;
  listening: Array<{ comprehensionCorrect?: boolean; transcriptOpened?: boolean; translationOpened?: boolean; slowAudioUsed?: boolean; replayCount?: number; unknownWords?: string[] }>;
  speaking: Array<{ pronunciationScore?: number; naturalnessScore?: number; fluencyScore?: number }>;
  vocabulary: Array<{ timesPracticed?: number; timesCorrect?: number; hits?: number; misses?: number }>;
  ...
  } satisfies ProficiencyAssessment["skillEvidence"];
  const evidenceCount = input.listening.length + input.speaking.length + vocabularyObservations + input.completedInteractions;
  const confidence: ProficiencyAssessment["confidence"] = evidenceCount < 6 ? "insufficient" : evidenceCount < 20 ? "early" : evidenceCount < 45 ? "developing" : "supported";
  ...
  const weakest = (Object.entries(skillEvidence) as Array<[LearningSkill, ProficiencyAssessment["skillEvidence"][LearningSkill]]>)
    .sort(([, a], [, b]) => (a.score ?? -1) - (b.score ?? -1))[0]?.[0];
  const nextCompetencies = CEFR_CURRICULUM
    .filter((item) => item.level === estimatedLevel && (item.skill === weakest || item.skill === "interaction"))
    .concat(CEFR_CURRICULUM.filter((item) => item.level === nextLevel && item.skill === weakest).slice(0, 1))
    .slice(0, 3);
