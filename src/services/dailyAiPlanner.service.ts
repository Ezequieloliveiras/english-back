import { AiDailyPlanBlueprint, ContentSemanticMetadata, DailyPlan, EnglishLevel } from "../types";
...
const localFallback = (plan: DailyPlan, level: EnglishLevel): AiDailyPlanBlueprint => ({
  ...
  activities: plan.blocks.slice(0, 5).map((block, index) => ({
    id: `${plan.id}:local-${block.type}-${index + 1}`, module: block.type === "speaking-coach" ? "pronunciation" : block.type === "review" ? "vocabulary" : block.type as any,
    contentMode: block.type === "review" ? "review" : "new", level,
...
export class DailyAiPlannerService {
  ...
  async create(plan: DailyPlan, state: LearningState) {
    if (!env.aiDailyPlanEnabled) return localFallback(plan, state.level);
    try {
      ...
      return { ...blueprint, activities: activities.map((activity) => ({ ...activity, id: `${plan.id}:${activity.id}` })) };
    } catch (error) {
      console.warn("[ai:daily-plan] using local fallback", error instanceof Error ? error.message : error);
      return localFallback(plan, state.level);
    }
  }
}
